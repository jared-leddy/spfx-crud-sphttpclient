          ...
          <div className={styles.row}>
            <div className={styles.column}>
              ...
              <div className={styles.buttonSection}>
                ...
                <div className={styles.button}>
                  <span className={styles.label} onClick={this.resetForm}>Reset</span>
                </div>
              </div>
            </div>
          </div>
...
// Get all items
  private getAllItems = (): void => {
    this.props.context.spHttpClient.get(this.props.context.pageContext.web.absoluteUrl + `/_api/web/lists/getbytitle('EmployeeDetails')/items?$select=Title,Age,ID`,
      SPHttpClient.configurations.v1,
      {
        headers: {
          'Accept': 'application/json;odata=nometadata',
      ...
      .then((response: SPHttpClientResponse) => {
        if (response.ok) {
          response.json().then((responseJSON) => {
            ...
            // console.log(responseJSON.value[0].Age);
          });
        } else {
          response.json().then((responseJSON) => {
            console.log(responseJSON);
